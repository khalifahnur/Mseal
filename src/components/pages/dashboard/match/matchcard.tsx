...
export function MatchCard({ match }: MatchCardProps) {
  ...
  return (
    <Card className={`flex flex-col ${match.isHome ? 'bg-blue-50' : 'bg-gray-50'}`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        {match.isHome ? <Trophy className="h-4 w-4 text-blue-500" /> : <Star className="h-4 w-4 text-yellow-500" />}
      </CardHeader>
      <CardContent>
        ...
        {isPastMatch ? (
          ...
          <div className="mt-4 text-center">
            <Badge 
              variant={match.ticketStatus === 'Available' ? 'default' : 'secondary'}
            >
              {match.ticketStatus}
            </Badge>
          </div>
        ...
        {isPastMatch ? (
          match.highlights && (
            <Button variant="outline" className="w-full" asChild>
              <a href={match.highlights} target="_blank" rel="noopener noreferrer">View Highlights</a>
            </Button>
          )
        ) : (
          <>
            <Button variant="outline" size="sm">
              {match.ticketStatus === 'Available' ? 'Book Tickets' : 'View Tickets'}
            </Button>
