...
export default function MatchesPage() {
  ...
  const fixturesQuery = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(`fixtures?team=${teamQuery.data}&season=${SEASON}`);
  ...
  // Process fixtures data when available
  if (fixturesQuery.data && !upcomingMatches.length && !pastMatches.length) {
    const matches: Match[] = fixturesQuery.data.map((fixture: any) => ({
      ...
      ticketStatus: new Date(fixture.fixture.date) > new Date() ? 'Available' : 'Closed',
      ...
      attended: false,
      highlights:
        fixture.fixture.status.short === 'FT' ? 'https://example.com/highlights' : null,
    }));

    const currentDate = new Date();
    setUpcomingMatches(matches.filter((match) => new Date(match.date) >= currentDate));
    setPastMatches(matches.filter((match) => new Date(match.date) < currentDate));
  }
  ...
  if (teamQuery.isError || fixturesQuery.isError) {
    return (
      <div className="text-red-500">
        Failed to fetch matches: {teamQuery.error?.message || fixturesQuery.error?.message}
      </div>
    );
  }
